...
merged <- read_delim("data/references/ncbi_merged_lookup.tsv",
                     delim = "|", trim_ws = TRUE,
                     col_names = c("old_tax_id",
                                   "new_tax_id",
                                   "blank")) %>%
    select(-blank)
...
metadata <- read_tsv("data/raw/rrnDB-5.6.tsv") %>%
    ...
    mutate(tax_id = ifelse(!is.na(new_tax_id), 
                           new_tax_id,
                           tax_id)) %>%
    select(-new_tax_id)
...
nodes <- read_delim("data/references/ncbi_nodes_lookup.tsv",
                    delim = "|",
                    trim_ws = TRUE,
                    col_names = c(
                        "tax_id",					
...
names <- read_delim("data/references/ncbi_names_lookup.tsv",
                    delim="|", trim_ws = TRUE,
                    col_names = c(
                        "tax_id",
                        "name_txt",
...
tree %>%
    ...
    pivot_longer(cols = starts_with("tr_"), names_to = "tr",
                 values_to = "id_rank") %>%
    select(-tr) %>%
